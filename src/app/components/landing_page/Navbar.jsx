...
const Navbar = ({ activeSection, setActiveSection, router }) => {
  ...
  return (
    <nav className="fixed w-full z-50 flex justify-between items-center p-2 md:p-3 bg-white/70 shadow-xl backdrop-blur-lg min-h-[60px] rounded-b-2xl border-b border-slate-200">
      <div className="text-base md:text-2xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 via-purple-500 to-pink-400 drop-shadow px-4 select-none">
        CardSmart
      </div>
      ...
      <div className="flex gap-2 items-center">
        ...
        <div className="hidden md:flex gap-2 items-center">
          ...
          <button
            className="px-4 py-2 border border-indigo-600 text-indigo-600 rounded-xl bg-white hover:bg-gradient-to-r hover:from-indigo-100 hover:to-pink-100 transition font-bold shadow-sm"
        ...
        <div className="md:hidden flex items-center">
          <button
            className="px-4 py-2 bg-gradient-to-r from-indigo-600 to-purple-500 text-white rounded-xl flex items-center gap-2 shadow-lg mx-4 hover:scale-105 transition-transform duration-200"
      ...
      {/* Mobile Nav Content */}
      {showDesktopNav && (
        <div className="flex flex-col items-start gap-2 bg-white/95 p-4 rounded-2xl shadow-2xl absolute top-14 right-4 z-50 w-56 border border-slate-200 animate-fade-in">
          {navItems.map((item) => (
            <button
              key={item.id}
              type="button"
