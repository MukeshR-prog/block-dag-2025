...
export async function POST(req) {
  try {
    ...
    const {
      ...
      amount,
      type,
      blockchain_hash, // ✅ For blockchain transactions
      recipient_address, // ✅ For crypto transactions
      network // ✅ For blockchain network info
    } = body;
    ...
    // 🔹 Create enhanced transaction document
    const transactionData = {
      ...
      network: network || "BlockDAG Testnet",
    ...
    if (cardData) {
      try {
        ...
        await updateDoc(doc(db, "card-details", card_id), {
          ...
          last_transaction_hash: blockchain_hash,
          updated_at: Timestamp.now()
        });
