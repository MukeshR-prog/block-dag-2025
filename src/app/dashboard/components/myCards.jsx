        ...
        <div className="relative h-full p-4 flex flex-col justify-between text-white">
          ...
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-2">
              ...
              {isPaymentCard ? (
                ...
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${iconBg || 'bg-white/20'}`}>
                  {Icon && <Icon className={`w-4 h-4 ${iconColor || 'text-white'}`} />}
                </div>
              )}
              
              <div className="space-y-0.5">
                <div className="flex items-center gap-1.5">
                  <h3 className="text-xs font-semibold text-white tracking-wide">{name}</h3>
                  {status && (
                    <div className="w-1 h-1 rounded-full bg-blue-300" />
                  )}
                </div>
                <p className="text-[10px] text-white/80 font-medium uppercase tracking-wider">{type}</p>
              </div>
            </div>
            ...
            <div className="flex items-center">
              {isPaymentCard ? (
                <div className="text-right">
                  <div className="text-white/90 font-bold text-sm tracking-wider font-serif">VISA</div>
                  {Icon && <Icon className="w-3 h-3 text-white/70 ml-auto mt-0.5" />}
                </div>
              ) : (
                onMenuClick && (
                  <button
          ...
          <div className="flex items-center justify-center my-2">
            {isPaymentCard ? (
              ...
              <div className="text-center">
                {usageCount && (
                  <p className="text-xs text-white/70 mb-1">Used {usageCount} times</p>
                )}
          ...
          <div className="space-y-2">
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <p className="text-base font-bold text-white tracking-tight">{formatAmount(balance)}</p>
              </div>
              
              {limit && isPaymentCard && (
                <div className="text-right">
                  <p className="text-[10px] text-white/70 uppercase tracking-wider font-medium mb-0.5">Credit Limit</p>
                  <p className="text-sm font-semibold text-white/95">{formatAmount(limit)}</p>
                </div>
              )}
            </div>
            ...
            {limit && isPaymentCard && (
              <div className="space-y-1">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] text-white/70 font-medium">Usage</span>
                  <span className="text-[10px] text-white/90 font-semibold">{usagePercentage.toFixed(0)}%</span>
                </div>
          ...
          <div className="flex items-end justify-between pt-2">
                ...
                <>
                  {expires && (
                    <div className="text-left">
                      <p className="text-[9px] text-white/60 uppercase tracking-wider font-medium">Valid Thru</p>
                      <p className="text-xs text-white/95 font-medium font-mono">{expires}</p>
                    </div>
                  )}
                  <div className="text-left">
                    <p className="text-[9px] text-white/60 uppercase tracking-wider font-medium">Cardholder</p>
                    <p className="text-xs text-white/95 font-medium">A. CITIZEN</p>
                  </div>
                </>
              ) : (
                <div className="text-left">
                  <p className="text-[9px] text-white/60 uppercase tracking-wider font-medium">Status</p>
                  <p className="text-xs text-white/95 font-medium capitalize">{status || 'Active'}</p>
                </div>
              )}
            </div>
            ...
            <Button
              ...
              className="group/btn bg-white/20 hover:bg-white/30 backdrop-blur-sm text-white text-xs font-medium px-3 py-1.5 rounded-lg transition-all duration-200 border border-white/30 hover:border-white/50"
            >
